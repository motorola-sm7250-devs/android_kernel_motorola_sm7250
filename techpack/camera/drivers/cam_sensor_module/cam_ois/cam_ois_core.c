...
void bu63169_calib_workaround(struct cam_ois_ctrl_t *o_ctrl)
{
	int rc = 0, i;
	...
	for(i = 0; i < ARRAY_SIZE(calib_reg_array); i++) {
		i2c_reg_setting.reg_setting = &calib_reg_array[i];

		rc = camera_io_dev_write(&o_ctrl->io_master_info, &i2c_reg_setting);
		if (rc < 0) {
			CAM_ERR(CAM_OIS, "%d bu63169 woraround failed", rc);
		}
	}
}
