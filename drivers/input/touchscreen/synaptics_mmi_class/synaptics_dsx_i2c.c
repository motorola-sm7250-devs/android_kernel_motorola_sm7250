				...
				struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = of_property_read_string(np, "synaptics,bound-display",
...
int synaptics_dsx_ic_reset(struct synaptics_rmi4_data *rmi4_data, int reset)
{
	...
	retval = request_irq(rmi4_data->irq, synaptics_dsx_reset_irq,
			IRQF_TRIGGER_RISING, rmi4_data->reset_name,
			rmi4_data);
	if (retval < 0) {
		...
		int state)
{
	...
	dev_info(&rmi4_data->i2c_client->dev, "state change %s -> %s\n",
...
static void TSI_state(struct synaptics_rmi4_data *rmi4_data,
			unsigned int tool, bool status)
{
	ud_log_status(&rmi4_data->touch_ud_stats, status);
	input_mt_report_slot_state(rmi4_data->input_dev, tool, status);
}

static void TSI_id(struct synaptics_rmi4_data *rmi4_data, int id)
{
	ud_set_id(&rmi4_data->touch_ud_stats, id);
	input_mt_slot(rmi4_data->input_dev, id);
}

#define INPUT_REPORT_MT_SLOT_STATE TSI_state
#define INPUT_REPORT_MT_SLOT TSI_id
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < rmi4_data->touch_ud_stats.ud_len; i++)
		total += scnprintf(buf + total, PAGE_SIZE - total,
				"[%d]: full cycles-%u, mismatch-%d\n", i,
				rmi4_data->touch_ud_stats.ud[i].counter,
				rmi4_data->touch_ud_stats.ud[i].mismatch);

	if (rmi4_data->button_0d_enabled) {
		total += scnprintf(buf + total, PAGE_SIZE - total,
			"buttons:\n");
		for (i = 0; i < rmi4_data->button_ud_stats.ud_len; i++)
			total += scnprintf(buf + total, PAGE_SIZE - total,
				"[%d]: full cycles-%u, mismatch-%d\n", i,
				rmi4_data->button_ud_stats.ud[i].counter,
				rmi4_data->button_ud_stats.ud[i].mismatch);
	}
	return total;
}
		...
		char *buf, ssize_t size)
{
	...
	for (i = 0; i < rmi4_data->touch_ud_stats.ud_len; i++)
		if (rmi4_data->touch_ud_stats.ud[i].mismatch)
			total += scnprintf(buf + total, size - total,
				"%d)%u,%d ", i,
				rmi4_data->touch_ud_stats.ud[i].counter,
				rmi4_data->touch_ud_stats.ud[i].mismatch);
		else if (rmi4_data->touch_ud_stats.ud[i].counter)
			total += scnprintf(buf + total, size - total,
				"%d)%u ", i,
				rmi4_data->touch_ud_stats.ud[i].counter);
	...
	if (rmi4_data->button_0d_enabled) {
		...
		for (i = 0; i < rmi4_data->button_ud_stats.ud_len; i++)
			if (rmi4_data->button_ud_stats.ud[i].mismatch)
				total += scnprintf(buf + total, size - total,
					"%d)%u,%d ", i,
					rmi4_data->button_ud_stats.ud[i].counter,
					rmi4_data->button_ud_stats.ud[i].mismatch);
			else if (rmi4_data->button_ud_stats.ud[i].counter)
				total += scnprintf(buf + total, size - total,
					"%d)%u ", i,
					rmi4_data->button_ud_stats.ud[i].counter);
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	struct synaptics_rmi4_data *rmi4_data =
					i2c_get_clientdata(to_i2c_client(dev));
	if (*buf == 's' || *buf == 'S') {
		rmi4_data->rctrl.reporting_stopped = true;
	} else if (*buf == 'r' || *buf == 'R') {
		rmi4_data->rctrl.reporting_stopped = false;
	} else {
		...
		rmi4_data->rctrl.reporting_stopped = value == 0;
	}
	if (rmi4_data->rctrl.reporting_stopped)
		rmi4_data->rctrl.events_cnt = 0;
	return count;
}
		...
		if (finger_status) {
			...
			if (rmi4_data->clipping_on && rmi4_data->clipa) {
				...
				if (inside == rmi4_data->clipa->inversion) {
					...
					INPUT_REPORT_MT_SLOT(rmi4_data, finger);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->tsb_buff_clean_flag) {
		memset(current_status, 0, sizeof(current_status));
#ifdef NO_0D_WHILE_2D
		memset(before_2d_status, 0, sizeof(before_2d_status));
		memset(while_2d_status, 0, sizeof(while_2d_status));
#endif
		rmi4_data->tsb_buff_clean_flag = 0;
	}
	...
	for (button = 0; button < f1a->valid_button_count; button++) {
		...
		ud_set_id(&rmi4_data->button_ud_stats, button);
		...
		if (rmi4_data->fingers_on_2d == false) {
			...
			input_report_key(rmi4_data->input_dev,
					f1a->button_map[button],
					status);
			ud_log_status(&rmi4_data->button_ud_stats, status == 1);
		} else {
...
static int synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (intr_status_mask & (~rmi4_data->active_fn_intr_mask)) {
		mutex_lock(&rmi4_data->exp_fn_ctrl.list_mutex);
		if (!list_empty(&rmi4_data->exp_fn_ctrl.fn_list)) {
			list_for_each_entry(exp_fhandler,
					&rmi4_data->exp_fn_ctrl.fn_list, link) {
		...
		mutex_unlock(&rmi4_data->exp_fn_ctrl.list_mutex);
	}
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	dev_dbg(&rmi4_data->i2c_client->dev,
			"%s: IRQ=%d; rmi4_data ptr = %p\n", __func__, irq, rmi4_data);
...
#if defined(CONFIG_DYNAMIC_DEBUG) || defined(DEBUG)
	/* Test mode can only be enforced in userdebug build */
	/* When test mode enforced, clear contiguous flag to */
	/* do two I2C transactions to retrieve touch data  */
	...
	if (rmi4_data->input_dev) {
		...
		*/
		...
		for (i = 0; i < rmi4_data->num_of_fingers; i++) {
			INPUT_REPORT_MT_SLOT(rmi4_data, i);
			INPUT_REPORT_MT_SLOT_STATE(rmi4_data,
	...
	if (need_to_query) {
		...
		if (det_workqueue)
			queue_delayed_work(det_workqueue,
					&rmi4_data->exp_fn_ctrl.det_work, 0);
	}
...
static void synaptics_rmi4_detection_work(struct work_struct *work)
{
	...
	struct synaptics_exp_fn_ctrl *exp_fn_ctrl =
			container_of(dw, struct synaptics_exp_fn_ctrl, det_work);
	struct synaptics_rmi4_data *rmi4_data =
...
static int synaptics_exp_fn_ctrl_init(
				struct synaptics_rmi4_data *rmi4_data)
{
	struct device *dev = &rmi4_data->i2c_client->dev;

	mutex_init(&rmi4_data->exp_fn_ctrl.ctrl_mutex);
	mutex_init(&rmi4_data->exp_fn_ctrl.list_mutex);
	INIT_LIST_HEAD(&rmi4_data->exp_fn_ctrl.fn_list);
	if (!det_workqueue) {
		det_workqueue = create_singlethread_workqueue("rmi_det_workqueue");
		if (!det_workqueue) {
			dev_err(dev, "%s: unable to create a workqueue\n", __func__);
			return -ENODEV;
		}
	}

	INIT_DELAYED_WORK(&rmi4_data->exp_fn_ctrl.det_work, synaptics_rmi4_detection_work);
	rmi4_data->exp_fn_ctrl.inited = true;
...
int synaptics_dsx_charger_mode(struct synaptics_rmi4_data *rmi4_data , int is_plugged)
{
	struct config_modifier *cm;
	int state;
	...
	state = synaptics_dsx_get_state_safe(rmi4_data);
	dev_info(&rmi4_data->i2c_client->dev,
		"power supply presence %d in state %d\n", is_plugged, state);
	...
	if (state == STATE_ACTIVE) {
		...
		if (!is_plugged) {
			...
			if (retval < 0)
				dev_err(&rmi4_data->i2c_client->dev,
					"power supply: sw reset failed %d\n", retval);
		}
		synaptics_dsx_sensor_ready_state(rmi4_data, false);
...
static int synaptics_rmi4_hw_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_dsx_ic_reset(rmi4_data, RMI4_HW_RESET);
	if (retval > 0)
		dev_dbg(&rmi4_data->i2c_client->dev, "%s: successful reset took %dms\n",
			__func__, retval);
	else
		dev_warn(&rmi4_data->i2c_client->dev, "%s: timed out waiting for idle\n",
			__func__);
		...
		const struct i2c_device_id *dev_id)
{
	...
	dev_dbg(&client->dev, "%s: rmi4_data ptr = %p\n", __func__, rmi4_data);
	...
	rc = synaptics_exp_fn_ctrl_init(rmi4_data);
	if (rc) {
		dev_err(&client->dev,
				"%s: Cannot alloc exp_fn_ctrl\n",
				__func__);
		goto free_and_exit;
	}
	...
	rmi4_data->irq_name = INSTANCE_RMI(DRIVER_NAME, rmi4_data);
	rmi4_data->reset_name = INSTANCE_RMI("synaptics_reset", rmi4_data);

	rc = synaptics_mmi_data_init(rmi4_data, true);
	if (rc < 0) {
		dev_err(&client->dev, "%s: Class init failure\n", __func__);
	}

	if (!rmi4_data->splash_screen_mode) {
		rc = synaptics_rmi4_hw_init(rmi4_data);
		if (rc < 0) {
			i2c_set_clientdata(rmi4_data->i2c_client, NULL);
			goto free_and_exit;
		}

		synaptics_rmi4_resume(&client->dev);
		dev_info(&client->dev, "%s: drm panel ready\n", __func__);
		dev_err(&client->dev, "%s: touch_stopped=%d, flash_enabled=%d\n", __func__,
				atomic_read(&rmi4_data->touch_stopped),
				rmi4_data->flash_enabled);
		return 0;
	}
